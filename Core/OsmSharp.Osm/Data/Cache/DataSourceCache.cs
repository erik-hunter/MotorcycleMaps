...
namespace OsmSharp.Osm.Data.Cache
{
    ...
    public class DataSourceCache : IDataSourceReadOnly
    {
        ...
        {
            var baseObjects = new List<OsmGeo>();
            TileRange range = TileRange.CreateAroundBoundingBox(box, _zoom_level);
            ...
            for (int x = range.XMin; x < range.XMax + 1; x++)
            {
                for (int y = range.YMin; y < range.YMax + 1; y++)
                {
                    var objectsInBox = IndexObjectsInBox(x, y);

                    baseObjects.AddRange(objectsInBox.Where(obj => obj.Shape.Inside(box)));
                }
            }
        ...
        {
            var baseObjects = new List<OsmGeo>();
            var objectsInBox = IndexObjectsInBox(tile.X, tile.Y);
            baseObjects.AddRange(objectsInBox.Where(obj => obj.Shape.Inside(tile.Box)));
            return baseObjects;
        }
        ...
        public IList<OsmGeo> Get(IList<Tile> tiles, Filter filter)
        {
            ...
            {
                baseObjects.AddRange(Get(tile, filter));
            }
            return baseObjects;
        }
