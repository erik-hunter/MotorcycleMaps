    ...
    public class OsmTagsIndex : ITagsIndex
    {
        ...
        public class OsmTags
        {
			...
			public override bool Equals(object obj)
			{
				if (!object.ReferenceEquals(this, obj))
				{
					if (obj is OsmTags)
					{
						...
						{
                            // first test all objects in the same order.
                            for (int idx1 = 0; idx1 < this._keys.Length; idx1++)
                            {
                                if (this._keys[idx1] != other._keys[idx1] ||
                                    this._values[idx1] != other._values[idx1])
                                {
                                    break;
                                }

                                if (idx1 == this._keys.Length - 1)
                                {
                                    return true;
                                }
                            }
			...
			public override int GetHashCode()
			{
			    if (_keys != null)
			    {
			        int hash = _keys.Length.GetHashCode();
