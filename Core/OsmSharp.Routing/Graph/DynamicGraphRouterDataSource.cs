...
namespace OsmSharp.Routing.Graph
{
        ...
        where TEdgeData : IDynamicGraphEdgeData
    {
        ...
        /// <returns></returns>
        public KeyValuePair<uint, KeyValuePair<uint, TEdgeData>>[] GetArcs(GeoCoordinateBox box)
        {
            ...
            return (from vertex in vertices
                    let localArcs = GetArcs(vertex)
                    from localArc in localArcs
                    select new KeyValuePair<uint, KeyValuePair<uint, TEdgeData>>(vertex, localArc)).ToArray();
        }
        ...
        public uint AddVertex(float latitude, float longitude, byte neighbours_estimate)
        {
            ...
            _vertexIndex.Add(new GeoCoordinate(latitude, longitude), vertex);
            return vertex;
        }
