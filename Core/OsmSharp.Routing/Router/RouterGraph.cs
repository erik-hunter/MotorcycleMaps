...
namespace OsmSharp.Routing.Router
{
    ...
    internal class RouterResolvedGraph
    {
        ...
        {
            var vertex = new RouterResolvedGraphVertex { Id = id, Latitude = latitude, Longitude = longitude };
        ...
        public void AddArc(long from, long to, RouterResolvedGraphEdge data)
        {
            ...
            KeyValuePair<long, RouterResolvedGraphEdge>[] arcs = vertex.Arcs;
            int idx;
            if (arcs == null)
            {
            ...
            arcs[idx] = new KeyValuePair<long, RouterResolvedGraphEdge>(to, data);
        ...
        public void DeleteArc(long from, long to)
        {
            ...
            if (_vertices.TryGetValue(from, out vertex))
            {
                KeyValuePair<long, RouterResolvedGraphEdge>[] arcs = _vertices[from].Arcs;
                if (arcs != null && arcs.Length > 0)
                {
                    var arcs_list = new List<KeyValuePair<long, RouterResolvedGraphEdge>>(arcs);
