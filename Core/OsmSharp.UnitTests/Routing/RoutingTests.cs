...
namespace OsmSharp.Osm.UnitTests.Routing
{
        ...
        where ResolvedType : IRouterPoint
    {
        ...
        /// <summary>
        /// Regression test for bug when routing using multiple resolved points between two vertices.
        /// 
        /// Tests that a router actually finds the shortest route.
        /// </summary>
        protected void DoTestShortestResolved3()
        {
            ...
            ResolvedType a = router.Resolve(VehicleEnum.Car, new GeoCoordinate(51.0581384, 3.7201161)); // a: 2 - a - b - c - 3
            ResolvedType b = router.Resolve(VehicleEnum.Car, new GeoCoordinate(51.0581685, 3.7201608)); // b: 2 - a - b - c - 3
            ResolvedType c = router.Resolve(VehicleEnum.Car, new GeoCoordinate(51.0581972, 3.7201964)); // c: 2 - a - b - c - 3

            OsmSharpRoute aTob = router.Calculate(VehicleEnum.Car, a, b);
            OsmSharpRoute aToc = router.Calculate(VehicleEnum.Car, a, c);
            OsmSharpRoute bToc = router.Calculate(VehicleEnum.Car, b, c);

            Assert.IsNotNull(aTob);
            Assert.IsNotNull(aToc);
            Assert.IsNotNull(bToc);
            Assert.IsTrue(aTob.TotalDistance < aToc.TotalDistance);
            Assert.IsTrue(bToc.TotalDistance < aToc.TotalDistance);
            Assert.AreEqual(2, aTob.Entries.Length);
            Assert.AreEqual(3, aToc.Entries.Length);
            Assert.AreEqual(2, bToc.Entries.Length);

            double[][] weights = router.CalculateManyToManyWeight(VehicleEnum.Car,
                new ResolvedType[] { a, b, c }, new ResolvedType[] { a, b, c });
            Assert.AreEqual(0, weights[0][0]);
            Assert.AreEqual(0, weights[1][1]);
            Assert.AreEqual(0, weights[2][2]);
            Assert.IsTrue(weights[0][2] > weights[0][1]);
            Assert.IsTrue(weights[1][0] > weights[1][1]);
            Assert.IsTrue(weights[1][2] > weights[1][1]);
            Assert.IsTrue(weights[2][0] > weights[2][1]);

            float latitude, longitude;
            data.GetVertex(2, out latitude, out longitude);
            ResolvedType vertex2 = router.Resolve(VehicleEnum.Car, new GeoCoordinate(latitude, longitude));
            data.GetVertex(3, out latitude, out longitude);
            ResolvedType vertex3 = router.Resolve(VehicleEnum.Car, new GeoCoordinate(latitude, longitude));

            OsmSharpRoute vertex2Toa = router.Calculate(VehicleEnum.Car, vertex2, a);
            OsmSharpRoute vertex2Tob = router.Calculate(VehicleEnum.Car, vertex2, b);
            OsmSharpRoute vertex2Toc = router.Calculate(VehicleEnum.Car, vertex2, c);

            Assert.IsNotNull(vertex2Toa);
            Assert.IsNotNull(vertex2Tob);
            Assert.IsNotNull(vertex2Toc);
            Assert.IsTrue(vertex2Toa.TotalDistance < vertex2Tob.TotalDistance);
            Assert.IsTrue(vertex2Tob.TotalDistance < vertex2Toc.TotalDistance);
            Assert.AreEqual(2, vertex2Toa.Entries.Length);
            Assert.AreEqual(3, vertex2Tob.Entries.Length);
            Assert.AreEqual(4, vertex2Toc.Entries.Length);

            weights = router.CalculateManyToManyWeight(VehicleEnum.Car,
                new ResolvedType[] { vertex2, a, b, c }, new ResolvedType[] { vertex2, a, b, c });

            OsmSharpRoute vertex3Toa = router.Calculate(VehicleEnum.Car, vertex3, a);
            OsmSharpRoute vertex3Tob = router.Calculate(VehicleEnum.Car, vertex3, b);
            OsmSharpRoute vertex3Toc = router.Calculate(VehicleEnum.Car, vertex3, c);

            Assert.IsNotNull(vertex3Toa);
            Assert.IsNotNull(vertex3Tob);
            Assert.IsNotNull(vertex3Toc);
            Assert.IsTrue(vertex3Toa.TotalDistance > vertex3Tob.TotalDistance);
            Assert.IsTrue(vertex3Tob.TotalDistance > vertex3Toc.TotalDistance);
            Assert.AreEqual(4, vertex3Toa.Entries.Length);
            Assert.AreEqual(3, vertex3Tob.Entries.Length);
            Assert.AreEqual(2, vertex3Toc.Entries.Length);

            weights = router.CalculateManyToManyWeight(VehicleEnum.Car,
                new ResolvedType[] { a, b, c, vertex3 }, new ResolvedType[] { a, b, c, vertex3 });

            data.GetVertex(11, out latitude, out longitude);
            ResolvedType vertex11 = router.Resolve(VehicleEnum.Car, new GeoCoordinate(latitude, longitude));
            data.GetVertex(4, out latitude, out longitude);
            ResolvedType vertex4 = router.Resolve(VehicleEnum.Car, new GeoCoordinate(latitude, longitude));

            OsmSharpRoute vertex11Toa = router.Calculate(VehicleEnum.Car, vertex11, a);
            OsmSharpRoute vertex11Tob = router.Calculate(VehicleEnum.Car, vertex11, b);
            OsmSharpRoute vertex11Toc = router.Calculate(VehicleEnum.Car, vertex11, c);

            Assert.IsNotNull(vertex11Toa);
            Assert.IsNotNull(vertex11Tob);
            Assert.IsNotNull(vertex11Toc);
            Assert.IsTrue(vertex11Toa.TotalDistance < vertex11Tob.TotalDistance);
            Assert.IsTrue(vertex11Tob.TotalDistance < vertex11Toc.TotalDistance);
            Assert.AreEqual(3, vertex11Toa.Entries.Length);
            Assert.AreEqual(4, vertex11Tob.Entries.Length);
            Assert.AreEqual(5, vertex11Toc.Entries.Length);

            weights = router.CalculateManyToManyWeight(VehicleEnum.Car,
                new ResolvedType[] { vertex11, a, b, c }, new ResolvedType[] { vertex11, a, b, c });

            OsmSharpRoute vertex4Toa = router.Calculate(VehicleEnum.Car, vertex4, a);
            OsmSharpRoute vertex4Tob = router.Calculate(VehicleEnum.Car, vertex4, b);
            OsmSharpRoute vertex4Toc = router.Calculate(VehicleEnum.Car, vertex4, c);

            Assert.IsNotNull(vertex4Toa);
            Assert.IsNotNull(vertex4Tob);
            Assert.IsNotNull(vertex4Toc);
            Assert.IsTrue(vertex4Toa.TotalDistance > vertex4Tob.TotalDistance);
            Assert.IsTrue(vertex4Tob.TotalDistance > vertex4Toc.TotalDistance);
            Assert.AreEqual(5, vertex4Toa.Entries.Length);
            Assert.AreEqual(4, vertex4Tob.Entries.Length);
            Assert.AreEqual(3, vertex4Toc.Entries.Length);

            weights = router.CalculateManyToManyWeight(VehicleEnum.Car,
                new ResolvedType[] { a, b, c, vertex4 }, new ResolvedType[] { a, b, c, vertex4 });
        }
