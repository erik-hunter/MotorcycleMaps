...
namespace OsmSharp.Osm.Data.SQLServer.SimpleSchema.Processor
{
    ...
    public class SQLServerSimpleSchemaDataProcessorTarget : DataProcessorTarget
    {
        ...
        public SQLServerSimpleSchemaDataProcessorTarget(string connectionString)
        {
        ...
        public SQLServerSimpleSchemaDataProcessorTarget(string connectionString, bool createSchema)
        {
            _connectionString = connectionString;
        ...
        private void BulkCopy(DataTable table, string tableName, int batchSize)
        {
            ...
            using (var bulkCopy = new SqlBulkCopy(_connection))
            {
                ...
                try
                {
                    Tools.Output.OutputStreamHost.Write("Inserting {0} records into {1}.", table.Rows.Count, tableName);
                    bulkCopy.WriteToServer(table);
                    Tools.Output.OutputStreamHost.WriteLine(" Done.");
                }
                catch (Exception e)
                {
                    Console.WriteLine(e);
        ...
        public override void AddNode(SimpleNode node)
        {
            ...
            nodeRow["tile"] = Tile.WorldToTilePos(node.Longitude.Value, node.Latitude.Value, TileDefaultsForRouting.Zoom).DatabaseId();

            // list the usr
            nodeRow["usr"] = node.UserName.ToStringEmptyWhenNull().Truncate(SQLServerSimpleSchemaConstants.NodeUsr);
            ...
            if (node.Tags != null)
            {
                foreach (KeyValuePair<string, string> tag in node.Tags)
                {
                    ...
                    tagRow["key"] = tag.Key.Trim().Truncate(SQLServerSimpleSchemaConstants.NodeTagsKey);
                    tagRow["node_id"] = id;
                    tagRow["value"] = tag.Value.Trim().Truncate(SQLServerSimpleSchemaConstants.NodeTagsValue);
        ...
        public override void AddWay(SimpleWay way)
        {
            ...
            // list the usr
            wayRow["usr"] = way.UserName.ToStringEmptyWhenNull().Truncate(SQLServerSimpleSchemaConstants.WayUsr);
            ...
            if (way.Tags != null)
            {
                ...
                {
                    string key = tag.Key.Truncate(SQLServerSimpleSchemaConstants.WayTagsKey);
                    ...
                    else
                    {
                        ...
                        tagRow["way_id"] = id;
                        tagRow["key"] = key;
                        tagRow["value"] = tag.Value.Truncate(SQLServerSimpleSchemaConstants.WayTagsValue);
        ...
        public override void AddRelation(SimpleRelation relation)
        {
            ...
            // list the usr
            relationRow["usr"] = relation.UserName.ToStringEmptyWhenNull().Truncate(SQLServerSimpleSchemaConstants.RelationUsr);
            ...
            if (relation.Tags != null)
            {
                foreach (KeyValuePair<string, string> tag in relation.Tags)
                {
                    ...
                    tagRow["key"] = tag.Key.Truncate(SQLServerSimpleSchemaConstants.RelationTagsKey);
                    tagRow["value"] = tag.Value.Truncate(SQLServerSimpleSchemaConstants.RelationTagsValue);
            ...
            if (relation.Members != null)
            {
                for (int idx = 0; idx < relation.Members.Count; idx++)
                {
                    ...
                    tagRow["member_role"] = member.MemberRole.Truncate(SQLServerSimpleSchemaConstants.RelationMemberRole);
                    tagRow["sequence_id"] = idx;
        ...
        public override void Close()
        {
            if(_connection != null)
            {
                Tools.Output.OutputStreamHost.WriteLine("Flushing remaining data");
                BulkCopy(_nodeTable, "node");
                BulkCopy(_nodeTagsTable, "node_tags");
                ...
                {
                    // Adds constraints
                    SQLServerSimpleSchemaTools.AddConstraints(_connection);
                }

                _connection.Close();
                _connection.Dispose();
                Tools.Output.OutputStreamHost.WriteLine("Database connection closed");
            }
            _connection = null;
        }
    }
